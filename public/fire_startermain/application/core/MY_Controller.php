...
class MY_Controller extends CI_Controller {
	...
	function add_css_theme( $css_files ){
		...
		foreach( $css_files as $css )
		{
			...
			$this->includes[ 'custom_css_files' ][ sha1( $css ) ] = base_url( "/themes/{$this->settings->theme}/css" ) . "/";
		}
	 ...
	 * @return	chained object
	 */
	 function add_custom_js_theme($js_files){
		// make sure that $this->includes has array value
		if ( ! is_array( $this->includes ) )
			$this->includes = array();
		...
		foreach( $js_files as $js ){
			...
			$this->includes[ 'custom_js_files' ][ sha1( $js ) ] = base_url( "/themes/{$this->settings->theme}/js" ) . "/";
		}
	...
	function add_js_theme( $js_files, $is_i18n = FALSE ){
		...
		foreach( $js_files as $js ){
			...
			// using sha1( $js ) as a key to prevent duplicate js to be included
			//$this->includes[ 'js_files' ][ sha1( $js ) ] = base_url( "/themes/{$this->settings->theme}/js" ) . "/";
		}
...
class Public_Controller extends MY_Controller {
    ...
    function __construct(){
		...
		$this
			...
			->add_custom_js_theme( "{$this->settings->theme}.js");
...
class Admin_Controller extends MY_Controller {
    ...
    function __construct()
    {
		...
		$this
			...
			->add_js_theme( "{$this->settings->theme}.js");
